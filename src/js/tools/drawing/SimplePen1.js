...
(function() {
  ...
  ns.SimplePen.prototype.draw = function(color, col, row, frame, overlay) { //Drawring
  ...
  ns.SimplePen.prototype.releaseToolAt = function(col, row, frame, overlay, event) {
    // apply on real frame
    var data = {col1: col,row1: row,frame1: frame,overlay1: overlay}
    socket.emit('penTool', data);
